...
#	- Case Insensitive
#	- Ignores ascii values not with ranges of upper and lower alpha ascii values
...
test_front_char:
	...
	b	front_is_proper			#t3 is now lower, branch to front_is_proper
front_not_upper:
	blt	$t3, 97, front_is_inproper	#if t3 < 97 it can't be lower
	bgt	$t3, 122, front_is_inproper	#if t3 > 122 it can't be lower
	b	front_is_proper			#t3 is lower, branch to front_is_proper
front_is_inproper:
	addu	$t1, $t1, 1			#since inproper increment t1 to next char
	b	test_loop			#Branch to test_loop (restart loop)
front_is_proper:				
...
test_back_char:
	...
	b	back_is_proper			#back is now lower, branch to back_is_proper
back_not_upper:
	blt	$t4, 97, back_is_inproper	#if t4 < 97 it can't be lower
	bgt	$t4, 122, back_is_inproper	#if t4 > 122 it can't be lower
	b	back_is_proper			#t4 is lower, branch to back_is_proper
back_is_inproper:
	subu	$t2, $t2, 1			#since inproper decrement t2 to next char
	b	test_loop			#Branch to test_loop (restart loop)
back_is_proper:
