...
main:
	...
	li	$t4, 10			#Initialize t4 as 10 (constant)
	
get_sign:
	li	$t3, 1			#Initialize t3 as 1 (used to indicate sign of number entered)
	lb	$t1, ($t0)		#Load the sign, if present, into t1
	bne	$t1, '-', positive	#t1 != '-', then branch to positive
	li	$t3, -1			#Change t3 to -1 to indicate negative if case
	addu	$t0, $t0, 1		#Increment to skip sign if negative
positive:
...
sum_loop:
	...
	blt	$t1, '0', end_sum_loop	#make sure t1 >= 0
	bgt	$t1, '9', end_sum_loop	#make sure t1 <= 9
	
	mult	$t2, $t4		#t2 *= 10
	mfhi	$t5			#check for overflow
	bnez	$t5, overflow		#if so, report an over flow
	mflo	$t2			#get the results of the mult and store in t2
...
end_sum_loop:
	...
	mul	$t2, $t2, $t3		#set the proper sign
...
overflow:
	la	$a0, overflow_str	#Load overflow_string address into a0
...
string_space:	.space	1024	#set aside 1024 bytes for an input string
	...
	.data
overflow_str	.asciiz	"Overflow Error\n\0"
